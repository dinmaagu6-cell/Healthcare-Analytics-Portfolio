# 👋 Welcome to My Healthcare Analytics Portfolio

Hi, I'm **Chidinma Agu**, a Registered Nurse transitioning into Healthcare Data Analytics.

I combine clinical experience with data analytics to transform healthcare data into meaningful insights that support evidence-based decision-making and improve patient outcomes.

---

## 🩺 About Me

- Registered Nurse (RN)
- Mental Health Nurse
- Ophthalmic Nurse
- Healthcare Data Analytics Enthusiast

---

## 💻 Technical Skills

- SQL (PostgreSQL)
- Power BI
- Microsoft Excel
- Data Visualization
- Healthcare Analytics
- Clinical Data Interpretation

---

## 📂 Projects

### 📊 Power BI

- Global HIV Analytics Dashboard *(WHO Dataset)*

### 🗄 SQL

- Lumina Healthcare Performance Analysis
- Skin Cancer Risk Intelligence & Patient Diagnosis Optimization *(Capstone)*

### 📈 Excel

- Healthcare KPI Dashboard
- Interactive Healthcare Reports

---

## 🎯 Career Goal

To leverage healthcare experience and data analytics skills to support data-driven clinical decisions, improve healthcare delivery, and contribute to better patient outcomes.

---

## 🤝 Connect With Me

- LinkedIn
- Medium
- Portfolio Website

Thank you for visiting my portfolio!
